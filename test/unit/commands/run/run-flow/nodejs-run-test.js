...
describe('Node run flow test', () => {
    ...
    it('| validate nodemon config, run mode', () => {
        ...
        stub.returns(true);
        stub.withArgs(path.join(process.cwd(), 'fooFolder', 'fooFileName.ts')).returns(false);
    ...
    it('| validate nodemon config, run mode [Typescript]', () => {
        const stub = sinon.stub(fs, 'existsSync');
        stub.returns(true);
        stub.withArgs(path.join(process.cwd(), 'fooFolder', 'fooFileName.ts')).returns(true);
        const runFlow = new NodejsRunFlow({ skillInvocationInfo,
            waitForAttach: false,
            debugPort: CONSTANTS.RUN.DEFAULT_DEBUG_PORT,
            token: 'fooToken',
        ...
        expect(nodemonConfig.args).to.deep.equal([
            ...
            'fooHandler',
            '--skillEntryFile',
            path.join(process.cwd(), 'fooFolder', 'fooFileName.ts'),
            '--region',
            CONSTANTS.ALEXA.REGION.NA
        ]);
        expect(nodemonConfig.watch).eq(false);
    });
    it('| validate nodemon config, debug mode, watch enabled [Typescript]', () => {
        const stub = sinon.stub(fs, 'existsSync');
        stub.returns(true);
        stub.withArgs(path.join(process.cwd(), 'fooFolder', 'fooFileName.ts')).returns(true);
        const runFlow = new NodejsRunFlow({ skillInvocationInfo,
        ...
        const nodemonConfig = runFlow.getExecConfig();
        expect(nodemonConfig.execMap).to.deep.equal({
            js: `node -r ts-node/register --inspect-brk=${CONSTANTS.RUN.DEFAULT_DEBUG_PORT}`
        });
        expect(nodemonConfig.script).eq(path.join(process.cwd(), 'fooFolder', CONSTANTS.RUN.NODE.SCRIPT_LOCATION));
        expect(nodemonConfig.args).to.deep.equal([
            '--accessToken',
